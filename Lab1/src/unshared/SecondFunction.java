...
public class SecondFunction implements Callable<double[]> {
    @Override
    public double[] call() {
        double[] temp1 = Operations.vectorMatrixMultiply(Data.B, Data.MT);
        double[] temp2 = Operations.vectorMatrixMultiply(Data.D, Data.MX);
        double[] temp3 = Operations.vectorScalarMultiply(temp2, Data.a);
        double[] res = Operations.vectorAddition(temp1, temp3);

        try (FileWriter bw = new FileWriter(".\\output_unsynchronized_C.txt")) {
            for (double elem: res) {
                bw.write(elem + "\n");
            }
            bw.write("\n\n\n");
